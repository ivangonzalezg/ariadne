...
// en una extensión comparable, que sí detecta estos reemplazos (confirmado inspeccionando
// su código).
//
...
1. Record a meeting again (same conditions as the prior quiet-mic report, ideally another A/B against a comparable extension) and confirm whether the user's own voice is now at a comparable volume to before, or still quiet.
