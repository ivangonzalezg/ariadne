...
- **Mecanismo de captura de audio (5.2, 5.3) — validado contra reuniones reales (ver sección 8):** en vez de las APIs de captura de pantalla/pestaña de Chrome (que siempre exigen una acción del usuario — ver más abajo), la captura automática de audio se logra interceptando el WebRTC interno de Google Meet desde un content script que corre en el contexto de la página (no en el mundo aislado habitual):
  ...
  - Que una extensión comparable (inspeccionada localmente) use exactamente esta misma técnica de mezcla (un `AudioContext`, un `MediaStreamAudioDestinationNode`, un solo archivo de salida) como su mecanismo principal fue evidencia de que el enfoque era viable en general — la implementación específica sí necesitó varias rondas de corrección tras validarse contra reuniones reales: (1) el manejo del ciclo de vida de las conexiones/tracks remotos era incompleto (limpieza al cerrar conexión nunca implementada, solo se reaccionaba al evento `"ended"` que no dispara de forma confiable cuando Meet reemplaza un track por renegociación), lo que producía eco por fuentes de audio remoto duplicadas; (2) las fuentes remotas se identificaban por conexión, y Meet reutiliza el mismo `MediaStream.id` de un participante al reconectar — corregido identificándolas globalmente por ese id (mismo esquema que usa esa extensión de referencia); (3) el mic propio se capturaba una sola vez vía `getUserMedia()` y nunca se actualizaba si Meet reemplazaba ese track más tarde (`RTCRtpSender.replaceTrack`) — esa extensión de referencia sí seguía ese reemplazo, y por eso su volumen de mic sonaba mejor; corregido replicando esa misma detección. Con las tres correcciones, validado por el usuario que audio y video se escuchan bien, sin eco, con el mic a un volumen normal.
...
- ~~La interceptación de WebRTC (5.2, 5.3, sección 6) depende de detalles internos de cómo Meet arma sus conexiones `RTCPeerConnection` (pueden ser varias, renegociarse, etc.) y debe validarse con una reunión real antes de darla por confiable~~ — validado: la implementación inicial sí tenía tres bugs reales, todos encontrados y corregidos contra reuniones reales (comparando en paralelo contra una extensión comparable) — ver el detalle en la sección 6. Con las tres correcciones aplicadas, el usuario confirmó audio y video correctos (sin eco, mic a volumen normal). Sigue existiendo el riesgo de mantenimiento de más largo plazo mencionado abajo (Meet puede cambiar cómo arma sus conexiones), pero la arquitectura en sí ya no es una incógnita.

**Abiertos:**
- La dependencia de la interfaz de Google Meet implica un riesgo de mantenimiento continuo si la plataforma cambia su estructura visual (subtítulos, botones de mic/colgar) o el modo en que arma sus conexiones `RTCPeerConnection` internamente. Esto no depende de nosotros y no tiene solución permanente — se acepta como mantenimiento continuo, con el fallback de 5.1 (si falla la transcripción, no bloquea audio/video) como mitigación, no como arreglo.
